...
@section('container')
    ...
    @if ($posts->count()) {{-- Hitung jumlah postingan = count() > 0 : True --}}
        <div class="card mb-3">
            @if($posts[0]->image)
            ...
            <div class="row">
                @foreach ($posts->skip(1) as $post)
                    <div class="col-md-4 mb-3">
                        <div class="card">
                            <div class="position-absolute px-3 py-2 text-white" style="
                                background-color:rgba(0, 0, 0, 0.7)"><a href="/posts?category={{ $post->category->slug }}" class="
                                text-decoration-none text-white">{{ $post->category->name }}</a>
                            </div>
                            @if($post->image)
